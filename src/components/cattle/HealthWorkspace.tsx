...
export function HealthWorkspace({
  ...
  const [vetName, setVetName] = useState("Dr. Farm Vet");
  ...
  function handleClinicalVisitSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        ...
        toast.error(res.error || "Failed to record clinical visit");
      }
    });
  }
  ...
  function handleGenerateCertificate() {
    startTransition(async () => {
      const res = await generateHealthCertificateAction(cattle.id, currentWeightKg, vetName || "Dr. Farm Veterinarian");
      if (res.success && res.certificate) {
        setCertificateData(res.certificate);
        setCertOpen(true);
      } else {
        toast.error(res.error || "Could not generate certificate");
      }
    });
  }
  ...
  function handleAddProtocol(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (res?.success) {
        toast.success("Health event scheduled successfully");
        setProtoOpen(false);
        setProtoTitle("");
        setProtoNotes("");
      } else if (res?.error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 rounded-2xl bg-card border border-border shadow-xs space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Health Status</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-base font-bold truncate">{cattle.is_quarantined ? "Quarantined" : "Healthy & Active"}</span>
          </div>
          <p className="text-xs text-muted-foreground">{cattle.is_quarantined ? "Sick pen isolation" : "Standard herd pen"}</p>
        </div>

        <div className="p-4 rounded-2xl bg-card border border-border shadow-xs space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Drug Withdrawal</span>
            <ShieldAlert className={cn("h-4 w-4", isUnderWithdrawal ? "text-amber-500" : "text-emerald-500")} />
          </div>
          <div className="text-base font-bold truncate">
            {isUnderWithdrawal ? (
              <span className="text-amber-600 dark:text-amber-400">Until {cattle.withdrawal_end_date}</span>
            ) : (
              <span className="text-emerald-600 dark:text-emerald-400">Clear (Fit for Sale)</span>
            )}
          </div>
          <p className="text-xs text-muted-foreground">{isUnderWithdrawal ? "Biological sales embargo" : "No active residues"}</p>
        </div>

        <div className="p-4 rounded-2xl bg-card border border-border shadow-xs space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Vaccine Schedule</span>
            <Syringe className="h-4 w-4 text-blue-500" />
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-black font-mono text-blue-600 dark:text-blue-400">{completedEvents.length}</span>
            <span className="text-xs text-muted-foreground">completed / {pendingEvents.length} pending</span>
          </div>
          <p className="text-xs text-muted-foreground">
            {overdueEvents.length > 0 ? (
              <span className="text-rose-500 font-semibold">{overdueEvents.length} overdue</span>
            ) : (
              "Up to date"
            )}
          </p>
        ...
        <div className="p-4 rounded-2xl bg-card border border-border shadow-xs space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Total Medical Spent</span>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  ...
                  Record Clinical Examination & Treatment
                </DialogTitle>
              </DialogHeader>
              <form onSubmit={handleClinicalVisitSubmit} className="space-y-4 pt-2">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <Select value={visitType} onValueChange={(v: any) => setVisitType(v)}>
                      ...
                      <SelectContent>
                        <SelectItem value="routine_check">Routine Health Check</SelectItem>
                        <SelectItem value="emergency">Emergency Treatment</SelectItem>
                        <SelectItem value="follow_up">Clinical Follow-up</SelectItem>
                        <SelectItem value="admission">Quarantine Admission</SelectItem>
                        <SelectItem value="discharge">Quarantine Discharge</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    <Label className="text-xs font-semibold">Veterinarian Name</Label>
                ...
                <div className="p-3 rounded-xl bg-muted/40 border border-border space-y-2">
                  <span className="text-xs font-bold uppercase text-muted-foreground flex items-center gap-1">
                    <Activity className="h-3.5 w-3.5" /> Bovine Vitals
                  </span>
                ...
                <div>
                  ...
                  <Input className="mt-1" placeholder="e.g. Mild Bovine Respiratory Disease" value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} required />
                </div>
                <div className="p-3 rounded-xl bg-muted/40 border border-border space-y-2">
                  <span className="text-xs font-bold uppercase text-muted-foreground flex items-center gap-1">
                    <Pill className="h-3.5 w-3.5" /> Medicine & Dosage
                  </span>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <div className="sm:col-span-2">
                      <Label className="text-[11px]">Medicine Item</Label>
                      <Select value={selectedMedId} onValueChange={(val: any) => handleMedSelect(val || "")}>
                        <SelectTrigger className="h-8 mt-1"><SelectValue placeholder="Select medicine..." /></SelectTrigger>
                        <SelectContent>
                          {medicineItems.map((m) => (
                    ...
                    <div>
                      <Label className="text-[11px]">Dose ({currentWeightKg > 0 ? `${currentWeightKg}kg` : "weight"})</Label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <div>
                    <Label className="text-xs font-semibold">Vet Fee (৳)</Label>
                    <Input type="number" min="0" value={vetFee} onChange={(e) => setVetFee(e.target.value)} className="font-mono mt-1" />
                  </div>
                  <div>
                    <Label className="text-xs font-semibold">Lab Fee (৳)</Label>
                ...
                <div>
                  ...
                  <Textarea placeholder="Recovery instructions..." value={recommendations} onChange={(e) => setRecommendations(e.target.value)} className="min-h-[50px] mt-1" />
                </div>
                ...
                <div className="flex items-center gap-2 p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20">
                  ...
                  <Label htmlFor="quar-check" className="text-xs font-semibold cursor-pointer text-amber-900 dark:text-amber-300">
                    Isolate in Quarantine Pen during treatment
                  </Label>
                </div>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  ...
                  Schedule Vaccine / Protocol
                </DialogTitle>
              </DialogHeader>
              <form onSubmit={handleAddProtocol} className="space-y-3 pt-2">
                <div>
                  <Label className="text-xs font-semibold">Title *</Label>
                  <Input placeholder="e.g. FMD Booster Vaccine" value={protoTitle} onChange={(e) => setProtoTitle(e.target.value)} required />
                </div>
                <div>
                  <Label className="text-xs font-semibold">Protocol Type</Label>
                ...
                <div>
                  ...
                  <Textarea placeholder="Dose or manufacturer details..." value={protoNotes} onChange={(e) => setProtoNotes(e.target.value)} className="min-h-[50px]" />
                </div>
                <div className="flex items-center justify-end gap-2 pt-2">
                  <Button type="button" variant="outline" onClick={() => setProtoOpen(false)}>Cancel</Button>
                  <Button type="submit" disabled={isPending}>Schedule Protocol</Button>
                </div>
              </form>
            </DialogContent>
          </Dialog>
        ...
        <div className="rounded-2xl bg-card border border-border shadow-xs overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            <h3 className="text-sm font-bold flex items-center gap-2">
              ...
              Clinical Treatment History & Prescriptions
            </h3>
          ...
          {treatments.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground">
              ...
              <p className="font-semibold text-sm">No medical treatments logged yet</p>
              <p className="text-xs mt-1">Record a clinical visit or medicine administration above.</p>
            </div>
          ) : (
                ...
                <div key={t.id} className="p-4 hover:bg-muted/20 transition-colors space-y-1.5">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-foreground">{t.diagnosis || "General Treatment"}</span>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                    ...
                    {t.vet_fee > 0 && <span>Vet Fee: {fmtBDT(t.vet_fee)}</span>}
        ...
        <div className="rounded-2xl bg-card border border-border shadow-xs overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            <h3 className="text-sm font-bold flex items-center gap-2">
              ...
              Vaccine & Routine Protocol Calendar
            </h3>
          ...
          {events.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground">
              ...
              <p className="font-semibold text-sm">No scheduled health events</p>
            </div>
          ) : (
            <div className="divide-y divide-border">
              {events.map((evt) => {
                        ...
                        <Button
                          ...
                          onClick={() =>
                            startTransition(async () => {
                              ...
                              toast.success("Protocol marked as completed");
                            })
                          }
                          disabled={isPending}
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Official Bovine Health Certificate & Clearance
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4 pt-2">
              <div className="p-4 rounded-2xl border border-border bg-card space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs">
                  <div>
                    <span className="text-muted-foreground font-medium">Tag ID:</span>
                    <p className="font-bold text-sm">#{certificateData.tagId}</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground font-medium">Breed / Gender:</span>
                    <p className="font-bold">{certificateData.breed} ({certificateData.gender})</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground font-medium">Live Weight:</span>
                    <p className="font-bold font-mono">{certificateData.currentWeightKg} kg</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground font-medium">Est. Age:</span>
                ...
                <div className="p-3 rounded-xl bg-muted/30 border border-border space-y-2">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold">Fit for Sale / Slaughter:</span>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold">Drug Withdrawal Status:</span>
                    <span>{certificateData.activeWithdrawalPeriod ? `Active until ${certificateData.withdrawalEndDate}` : "Cleared"}</span>
                  </div>
                </div>
                ...
                <div className="pt-2">
                  ...
                  {certificateData.completedVaccinations.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic">No completed vaccines logged.</p>
                  ) : (
                ...
                <div className="pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
                  <span>Certified by: <strong className="text-foreground">{certificateData.certifiedBy}</strong></span>
      ...
      <ConfirmDialog
        ...
        title="Delete Health Protocol"
        description="Are you sure you want to remove this scheduled health event?"
        confirmLabel="Delete"
        destructive={true}
